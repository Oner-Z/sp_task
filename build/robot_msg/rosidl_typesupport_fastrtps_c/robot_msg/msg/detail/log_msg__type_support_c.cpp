...
#include "robot_msg/msg/detail/log_msg__rosidl_typesupport_fastrtps_c.h"
...
#include "rosidl_runtime_c/string.h"  // diagnostic_info, node_name
#include "rosidl_runtime_c/string_functions.h"  // diagnostic_info, node_name
...
using _LogMsg__ros_msg_type = robot_msg__msg__LogMsg;
  ...
  eprosima::fastcdr::Cdr & cdr)
{
  ...
  // Field name: diagnostic_info
  {
    const rosidl_runtime_c__String * str = &ros_message->diagnostic_info;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: diagnostic_info
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->diagnostic_info.data) {
      rosidl_runtime_c__String__init(&ros_message->diagnostic_info);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->diagnostic_info,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'diagnostic_info'\n");
      return false;
    }
  }
...
size_t get_serialized_size_robot_msg__msg__LogMsg(
  const void * untyped_ros_message,
  size_t current_alignment)
{
  const _LogMsg__ros_msg_type * ros_message = static_cast<const _LogMsg__ros_msg_type *>(untyped_ros_message);
  ...
  current_alignment += padding +
    eprosima::fastcdr::Cdr::alignment(current_alignment, padding) +
    (ros_message->diagnostic_info.size + 1);
...
static uint32_t _LogMsg__get_serialized_size(const void * untyped_ros_message)
{
  return static_cast<uint32_t>(
    get_serialized_size_robot_msg__msg__LogMsg(
      untyped_ros_message, 0));
}

ROSIDL_TYPESUPPORT_FASTRTPS_C_PUBLIC_robot_msg
size_t max_serialized_size_robot_msg__msg__LogMsg(
  bool & full_bounded,
  bool & is_plain,
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = robot_msg__msg__LogMsg;
    is_plain =
      (
      offsetof(DataType, diagnostic_info) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _LogMsg__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_robot_msg__msg__LogMsg(
...
const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, robot_msg, msg, LogMsg)() {
